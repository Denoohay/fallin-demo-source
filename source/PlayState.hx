...
class PlayState extends MusicBeatState
{
	...
	public static var blanks:Int = 0;
	...
	override public function create()
	{
		...
		switch(SONG.stage)
		{
			...
			case 'theWhirlygig':
				{
					curStage = 'theWhirlygig';

					defaultCamZoom = 0.6;

					var bg:FlxSprite = new FlxSprite(-1000, -500).loadGraphic(Paths.image('FGstages/sky'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.2, 0.2);
					bg.active = false;
					bg.setGraphicSize(Std.int(bg.width * 0.8));
					...
					var bgEscalator:FlxSprite = new FlxSprite(-1100, -600).loadGraphic(Paths.image('FGstages/wback'));
					bgEscalator.antialiasing = true;
					bgEscalator.scrollFactor.set(0.3, 0.3);
					bgEscalator.active = false;
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					...
					var fgSnow:FlxSprite = new FlxSprite(-800, 620).loadGraphic(Paths.image('FGstages/wground'));
					fgSnow.active = false;
					fgSnow.antialiasing = true;
					add(fgSnow);

					bottomBoppers = new FlxSprite(-300, 140);
					bottomBoppers.frames = Paths.getSparrowAtlas('FGstages/wbop');
					bottomBoppers.animation.addByPrefix('bop', 'fallbopw', 24, false);
					bottomBoppers.antialiasing = true;
			...
			case 'seeSaw':
				{
					...
					var bgEscalator:FlxSprite = new FlxSprite(-1100, -600).loadGraphic(Paths.image('FGstages/ssback'));
					bgEscalator.antialiasing = true;
					bgEscalator.scrollFactor.set(0.8, 0.8);
					bgEscalator.active = false;
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					bgEscalator.updateHitbox();
					add(bgEscalator);

					var fgSnow:FlxSprite = new FlxSprite(-800, 292).loadGraphic(Paths.image('FGstages/ssground'));
					fgSnow.active = false;
					fgSnow.antialiasing = true;
					add(fgSnow);

					bottomBoppers = new FlxSprite(-300, 140);
					bottomBoppers.frames = Paths.getSparrowAtlas('FGstages/ssbop');
					bottomBoppers.animation.addByPrefix('bop', 'fallbopss', 24, false);
					bottomBoppers.antialiasing = true;
					bottomBoppers.scrollFactor.set(1, 1);
					bottomBoppers.setGraphicSize(Std.int(bottomBoppers.width * 1));
					bottomBoppers.updateHitbox();
			...
			case 'Ring':
				{
					...
					var bg:FlxSprite = new FlxSprite(-435, -50).loadGraphic(Paths.image('FGstages/ring'));
					bg.antialiasing = true;
					...
					var bgEscalator:FlxSprite = new FlxSprite(1250, -80).loadGraphic(Paths.image('FGstages/Calobi'));
					bgEscalator.antialiasing = true;
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - KE v" + MainMenuState.kadeEngineVer : ""), 16);
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			{
				case 0:
					var starting:FlxSprite = new FlxSprite().loadGraphic(Paths.image(introAlts[0]));
					starting.antialiasing = true;
					starting.scrollFactor.set();
					starting.updateHitbox();

					if (curStage.startsWith('school'))
						starting.setGraphicSize(Std.int(starting.width * daPixelZoom));

					starting.screenCenter();
					add(starting);
					FlxTween.tween(starting, {y: starting.y += 100, alpha: 0}, Conductor.crochet / 1000, {
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ENTER && startedCountdown && canPause)
		{
			...
			if (FlxG.random.bool(0.1))
			{
				...
				openSubState(new GitarooPause(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
			}
			else
				openSubState(new PauseSubState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
		}

		if (FlxG.keys.justPressed.ESCAPE && startedCountdown && canPause)
		{
			persistentUpdate = false;
			persistentDraw = true;
			paused = true;
	...
	function endSong():Void
	{
		...
		else
		{
			{
				{
					if (curSong == 'Rap Battle') {
						FlxG.switchState(new StupidState());

					transIn = FlxTransitionableState.defaultTransIn;
					transOut = FlxTransitionableState.defaultTransOut;

					switch (FlxG.save.data.eeeeeeeeeeeeeeeeeeeeeeeeeeeeee)
					{
						case true:
							{
							}
						case false:
							{
								if (SONG.song.toLowerCase() == 'rap battle') {
									FlxG.switchState(new StupidState());
								} else {
									FlxG.switchState(new StupidState());
								}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (curSong == 'Earthquake') {
						video.playMP4(Paths.video('cut2'), new DemoSubState()); 
						isCutscene = true;
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			if (curStage.startsWith('Ring'))
			{
				pixelShitPart1 = 'shared';
				pixelShitPart2 = 'blank';
			}
